...
from .latent_heads import MechanisticLatentHeads, SharedLatentHeads
...
class IPTNet(nn.Module):
    def __init__(self, config: dict) -> None:
        ...
        self.graph = GraphEncoder3D(
            ...
            bool(m.get("strip_edge_geometry", False)),
        )
        self.use_interaction_encoder = bool(m.get("use_interaction_encoder", True))
        self.interaction = InteractionEncoder(hidden, dropout)
        self.project = StructureProjector(hidden, dropout, bool(m.get("use_pairwise_ion_features", True)))
        ...
        if bool(m.get("use_mechanistic_latent_heads", True)):
            self.latents = MechanisticLatentHeads(
                hidden,
                latent,
                dropout,
                bool(m.get("use_property_latent_gating", True)),
                bool(m.get("zero_property_latents", False)),
            )
        else:
            self.latents = SharedLatentHeads(hidden, latent, dropout)
    ...
    def forward(self, batch):
        ...
        h_inter = self.interaction(h_cation, h_anion) if self.use_interaction_encoder else h_cation.new_zeros(h_cation.shape)
