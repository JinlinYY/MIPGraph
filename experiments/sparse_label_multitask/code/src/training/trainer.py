...
class Trainer:
    def __init__(
        ...
        self.split = split
        self.train_label_mask = None if train_label_mask is None else train_label_mask.astype("float32")
        loss_cfg = config["loss"]
        if scalers is None:
            scaler_arrays = arrays
            if self.train_label_mask is not None:
                scaler_arrays = dict(arrays)
                scaler_mask = arrays["mask"].copy()
                train_idx = split["train"]
                scaler_mask[train_idx] = self.train_label_mask[train_idx]
                scaler_arrays["mask"] = scaler_mask
    ...
    def _fallback_metric_value(self, metrics: dict, metric_name: str):
        candidates = []
        if metric_name == "focus_macro_log_R2":
            candidates = ["focus_macro_R2", "macro_log_R2", "macro_R2"]
        elif metric_name == "focus_macro_R2":
            candidates = ["focus_macro_log_R2", "macro_R2", "macro_log_R2"]
        elif metric_name == "focus_macro_log_MAE":
            candidates = ["focus_macro_MAE", "macro_log_MAE", "macro_MAE"]
        elif metric_name == "focus_macro_MAE":
            candidates = ["focus_macro_log_MAE", "macro_MAE", "macro_log_MAE"]
        for candidate in candidates:
            value = self._metric_value(metrics, candidate)
            if value is not None:
    ...
    def loader(self, split_name: str, shuffle: bool = False):
        ds = ILPropertyDataset(
            ...
            self.y_scaled,
            self.error_weights,
            self.train_label_mask if split_name == "train" else None,
        )
        ...
        with log_path.open("w", newline="", encoding="utf-8") as f:
            ...
            for epoch in range(1, self.config["training"]["epochs"] + 1):
                ...
                for batch in tqdm(train_loader, desc=f"Epoch {epoch} train"):
                    ...
                    if float(batch.mask.sum().detach().cpu()) <= 0.0:
                        continue
                ...
                if should_validate:
                    ...
                    if monitor_score is None:
                        monitor_score, fallback_metric = self._fallback_metric_value(val_metrics, monitor_metric)
                        if monitor_score is None:
                            print(f"warning: monitor metric {monitor_metric!r} was not available; using train_loss for this epoch")
                            monitor_score = sum(losses) / max(len(losses), 1)
                        else:
                            print(f"warning: monitor metric {monitor_metric!r} was not available; using {fallback_metric!r}")
                else:
