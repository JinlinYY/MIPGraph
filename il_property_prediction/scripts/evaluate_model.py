...
def main():
    ...
    bootstrap_cfg = load_config(args.config)
    bootstrap_base = bootstrap_cfg["_base_dir"]
    ckpt = torch.load(resolve_path(args.checkpoint, bootstrap_base), map_location="cpu", weights_only=False)
    cfg = ckpt.get("config", bootstrap_cfg)
    base = cfg.get("_base_dir", bootstrap_base)
    ...
    configured_split = cfg["data"].get("split_path")
    split_path = (
        resolve_path(configured_split, base)
        if configured_split
        else resolve_path(cfg["data"]["processed_dir"], base)
        / "splits"
        / f"{cfg['data'].get('split_type','il_level')}_seed{cfg['data']['seed']}.json"
    )
    ...
    save_metrics_csv(metrics, metrics_path.with_name(f"{args.split}_metrics_raw.csv"), cfg["properties"]["names"])
