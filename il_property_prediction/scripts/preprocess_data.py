...
def main():
    ...
    evaluation_reference = resolve_path(args.evaluation_reference, base) if args.evaluation_reference else None
    result = preprocess_excel(
        raw_path,
        processed_dir,
        cfg["data"].get("sheet_name", "Merged"),
        cfg["properties"]["names"],
        evaluation_reference_path=evaluation_reference,
    )
