...
class GraphEncoder3D(nn.Module):
    def __init__(
        ...
        self.edge_type_index = int(edge_type_index)
    ...
    def _geometry_features(self, data) -> torch.Tensor:
        ...
        # Current edge_attr layout from graph_featurizer.bond_features:
        # 0:4 bond type, 4 conjugated, 5 ring, 6:9 stereo, 9 edge_type.
        # Old pre-MIPGraphNet checkpoints used index 6 by mistake; MIPGraphNet
        # can still load those checkpoints by passing edge_type_index=6.
        edge_type = data.edge_attr[:, self.edge_type_index] if data.edge_attr.size(1) > self.edge_type_index else torch.zeros_like(distance)
